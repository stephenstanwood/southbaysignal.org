...
### Context
Monday September 7, 2026. Roadmap still closed (6/6). The plan for the cycle was
the Sep 3–7 funding window; the funding work happened, but the bigger find was
that the homepage's core feature had been dark for eight days.

### What Was Built

**The Today tab's day plans were empty for every visitor since August 31, and
the cause was a launchd job that could no longer start.** `default-plans.json`
carried `generatedAt: 2026-08-30`, and `SouthBayTodayView` opens its visible-card
computation with `if (planDateISO < todayPT) return []` — a plan dated in the
past renders zero cards. So the headline asked "What should we do today?" and the
grid beneath it had nothing in it, every day, for eight days. The stale plan it
was refusing to show was pointing at an August 30 San Jose Giants game against
Visalia and an August 30 El Tri show, so failing closed was the right behavior;
the problem was upstream.

`org.southbaytoday.default-plans-refresh` was still loaded and still scheduled
for 3:20 AM, but `launchctl list` reported **exit 78 (EX_CONFIG)** and
`~/Library/Logs/default-plans-refresh.log` had not been written to since
2026-08-30 03:21 — the job was never spawning, so it never got far enough to log
a failure. `launchctl print` listed `needs LWCR update` in the job's properties,
and `/opt/homebrew/bin/node` had been relinked to Cellar 26.8.1 at **Aug 30
03:20** — the same minute as the last successful run. launchd was holding a code
requirement for a node binary that no longer existed, and refused to launch the
replacement. `org.southbaytoday.photoref-refresh` was sitting at exit 78 for the
same reason. Both were repaired with a `launchctl bootout` + `bootstrap` cycle,
which re-registers the requirement against the current binary; both now report
exit 0.

Plans were then regenerated by hand for Sep 7 and Sep 8 — six adults cards and
six kids cards for each day — so the Today tab has answers again without waiting
for tomorrow's 3:20 AM window.

The generic lesson, worth keeping: a Homebrew upgrade of a binary a LaunchAgent
invokes directly can silently kill that agent, and the failure is invisible in
the job's own log because the job never runs. `launchctl list | grep 78` is the
check.

**TabaPay's $155M is on the Tech tab, and its city label was resolved against
primary sources.** The Palo Alto payments-infrastructure company — fifth-largest
card-not-present processor in the U.S. by transaction count, on track to move
more than $100 billion this year — closed a $155M growth financing from FTV
Capital on September 2 and announced it intends to buy Transact Bank, an
OCC-chartered bank in Denver, rename it TabaBank, and put both under a new
holding company. A national charter would give it direct Fed rails including
FedNow and one federal regulator instead of the state-by-state money-transmitter
licenses it runs on now.

FinSMEs, PitchBook, and ZoomInfo all file TabaPay under Mountain View; both
primary sources say Palo Alto (the FTV-issued release datelines "PALO ALTO,
Calif. — September 2, 2026," and the California DFPI money-transmitter license,
ID 2769, lists 450 Cambridge Ave). Both cities are in coverage, so only the card
label was at stake — but the same aggregator-vs-primary rule that governs the
in/out calls governs this one too.

Two near-misses are now recorded in the file so the next cycle doesn't re-verify
them: **WindBorne Systems** ($37M Series B, Aug 5) is tagged "Palo Alto, CA-based"
by FinSMEs but its own announcement datelines Redwood City and says only that it
was *founded* in Palo Alto — San Mateo County, same exclusion as Fireworks AI,
Cognichip, and Array Labs. **Thinking Machines Lab** (~$1B reportedly in talks,
Sep 3) is San Francisco, and a round in talks is not a closed round.

Primary sources:
- https://ftvcapital.com/2026/tabapay-closes-155-million-strategic-growth-financing-led-by-ftv-capital-and-announces-planned-acquisition-of-transact-bank/
- https://dfpi.ca.gov/regulated_entity/tabapay-payment-services-llc/
- https://windbornesystems.com/blog/windborne-systems-raises-37-million-to-build-the-worlds-weather-intelligence-infrastructure

### Verification

`npm test` (11/11), `npx astro check` (0 errors, 0 warnings), and `npm run build`
all pass, including the locked-Home prebuild gate. Confirmed in the built output
that the TabaPay card renders in `/tech`'s server HTML. `default-plans.json` now
reads `generatedAt: 2026-09-07` with four populated plans (today + tomorrow,
adults + kids). Both repaired LaunchAgents report exit 0 after the bootstrap.

The audit surfaces nothing needing a fix this cycle: `events-suspected-issues.json`
shows 0 hard findings across 1,856 upcoming and 313 inbound events, and a sweep
of all 1,861 events for mojibake, HTML entities, all-caps titles, unbalanced
parens, placeholder text, and true duplicates (same title, date, venue, time)
came back empty except for one Gamble Garden workshop with neither description
nor blurb.
...
`src/data/south-bay/elections-2026.ts` is an orphan — nothing imports it.
`ElectionsCard` was deleted in 736db7f1 ("chore: delete orphaned SportsView,
DevelopmentView, ElectionsCard"), so per Rule #1 this is forbidden territory, not
a bug to fix. Worth knowing that the data inside it is also stale in its own
right: it covers only the June 2 primary and its polling note is dated late
April. Do not wire it in. (The three unrendered Tech tab sections and
`tech-briefing.json` from cycle 218's note are all still orphans too.)

One event in `upcoming-events.json` carries `city: "monte-sereno"`, which is not
one of the 12 slugs in the `City` union — `cityLabel()` title-cases the fallback
so the card reads "Monte Sereno" correctly, but the Events city filter has no
chip for it, so that event is only reachable with all cities selected. One row
today; not worth widening the type for.
